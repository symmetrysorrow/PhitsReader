...
std::tuple<double, double, int> peak_c(const std::vector<double>& data, int presamples, int w_max, int x_av, int w_av) {
    ...
    double peak = *peak_iter;  // 最大値
    int peak_index = std::distance(data.begin(), peak_iter);  // 最大値のインデックス

    // 平均値を計算 (peak_index - x_av から w_av の範囲)
    int av_start = peak_index - x_av;
    double peak_av = std::accumulate(data.begin() + av_start, data.begin() + av_start + w_av, 0.0) / w_av;

    // 結果を返す (ピーク値、周辺の平均値、ピークのインデックス)
...
Eigen::VectorXd coder_array_to_eigen(const coder::array<double, 2U>& coder_arr) {
    ...
    // coder::array から Eigen ベクトルへのデータコピー
    for (int i = 0; i < coder_arr.size(0); ++i) {
        eigen_vec(i) = coder_arr[i]; // 二次元配列のため列インデックスは0
    }
...
std::tuple<double, int, int> risetime(const std::vector<double>& data, double peak, int peak_index, double rise_high, double rise_low, double rate) {
    ...
    // peak_index から逆方向にスキャンし、rise_high に相当する位置を探す
    ...
    // rise_90 から逆方向にスキャンし、rise_low に相当する位置を探す
    ...
    // 結果を返す (ライズタイム、rise_10、rise_90 のインデックス)
    return { rise, rise_10, rise_90 };
}

// Eigen::VectorXd を coder::array<double, 2U> に変換する関数
void eigen_to_coder_array(const Eigen::VectorXd& eigen_vec, coder::array<double, 2U>& coder_arr) {
    // coder::array のサイズを設定
    coder_arr.set_size(eigen_vec.size(), 1);  // (行数, 列数)

    // Eigen ベクトルから coder::array へのデータコピー
    for (int i = 0; i < eigen_vec.size(); ++i) {
        coder_arr[i + 1] = eigen_vec(i); // インデックスを1始まりに調整
    }
}

std::tuple<int, double, double> MakeCSV(const Eigen::VectorXd& Vector,const InputParameters& InputPara,const int& Event)
{
    int N = 2;
    ...
    bd.set_size(1, 3);  // 行列のサイズを設定 (行, 列)
    ad.set_size(1, 3);  // 行列のサイズを設定 (行, 列)
